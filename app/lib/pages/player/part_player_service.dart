...
class MusicPlayer implements ValueNotifier<PlayerControllerState> {
  MusicPlayer._private() : super() {
    () async {
      ...
      try {
        current =
            null; //Music.fromMap(json.decode(preference.get(_PREF_KEY_PLAYING)));
        token = "token_blind_english"; //preference.get(_PREF_KEY_TOKEN);
        playingList = null;
...
//            .toList();
  ...
  Future<void> changePlayMode() {
    ...
    PlayMode next = PlayMode.values[(value.playMode.index + 1) % 2];
